...
class GameLibrary {
	...
	async scanGameFolders() {
		...
		try {
			// Fetch the games folder content from GitHub
			const gamesFolderUrl = 'https://raw.githubusercontent.com/vouimet109-png/Crokette-Launcher/main/games/';
			
			// Try to get list from a manifest if it exists
			try {
				...
				console.log('[gamecode] no games.json manifest found, trying directory scan');
			}
			
			// Alternative: try GitHub API to list directories
			try {
				...
				if (apiResp.ok) {
					...
					console.log('[gamecode] found folders from GitHub API:', folders);
				...
				console.log('[gamecode] GitHub API failed:', e);
			}
			
			console.warn('[gamecode] could not find any games');
		} catch (e) {
