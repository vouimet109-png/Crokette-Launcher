...
class GameLibrary {
	...
	async scanGameFolders() {
		...
		// First, try the GitHub API (most reliable for dynamic detection)
		try {
			...
			if (apiResp.ok) {
				...
				const folders = contents.filter(item => item.type === 'dir').map(item => item.name).sort();
		...
		// Fallback: try index.txt if API fails
		...
		console.warn('[gamecode] falling back to hardcoded list');
