...
class GameLibrary {
	...
	async scanGameFolders() {
		...
		try {
			// Fallback: hardcoded list of games
			const gameList = ['mario'];
			console.log('[gamecode] loading games from hardcoded list:', gameList);
			
			for (const folderName of gameList) {
				const game = await this.loadGameMetadata(folderName);
				if (game) {
					console.log('[gamecode] game loaded:', game);
	...
	async loadGameMetadata(folderName) {
		try {
			...
			const jsonUrl = `${base}${folderName}/github.json`;
