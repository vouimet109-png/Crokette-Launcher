...
class GameLibrary {
	...
	async scanGameFolders() {
		...
		try {
			...
			if (!manifestResp.ok) {
				console.warn('[gamecode] manifest not found at', manifestUrl, 'status', manifestResp.status);
				// On GitHub Pages the directory listing is not available, so we stop here
				return;
			}
			...
			console.error('[gamecode] error fetching manifest at', base + 'games.json', e);
		}
	}
