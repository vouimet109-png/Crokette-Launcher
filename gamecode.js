...
class GameLibrary {
	...
	async scanGameFolders() {
		try {
			// Fetch the game directory listing
	...
	async loadGameMetadata(folderName) {
		try {
			...
			const coverUrl = `game/${folderName}/cover.png`;
			const coverResponse = await fetch(coverUrl);
			const hasCover = coverResponse.ok;
