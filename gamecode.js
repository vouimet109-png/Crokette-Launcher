...
class GameLibrary {
	...
	async scanGameFolders() {
		...
		try {
			// Try GitHub API to list directories
			console.log('[gamecode] trying GitHub API to detect game folders');
			...
			if (apiResp.ok) {
				...
				console.log('[gamecode] GitHub API response:', contents);
				
				const folders = contents
					.filter(item => item.type === 'dir')
					.map(item => item.name);
				
				console.log('[gamecode] found game folders:', folders);
				
				if (folders.length > 0) {
					for (const folderName of folders) {
						console.log('[gamecode] loading game:', folderName);
						const game = await this.loadGameMetadata(folderName);
						if (game) {
							console.log('[gamecode] game loaded successfully:', game.name);
							this.games.push(game);
						} else {
							console.warn('[gamecode] failed to load game:', folderName);
						}
					}
		...
		console.log('[gamecode] fallback: trying common game folders');
		// Fallback: try to load common folders manually
		const commonFolders = ['mario', 'tetris', 'snake', 'pacman', 'flappy-bird', 'breakout'];
		for (const folderName of commonFolders) {
			try {
				const game = await this.loadGameMetadata(folderName);
				if (game) {
					console.log('[gamecode] fallback game loaded:', game.name);
					this.games.push(game);
				}
			} catch (e) {
				// ignore, try next
